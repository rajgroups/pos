...
@section('content')
    <div class="page-header">
        <div class="add-item d-flex">
            <div class="page-title">
                <h4 class="fw-bold">Edit Vehicle Document</h4>
                <h6 class="text-muted">Update vehicle document details or verification status</h6>
            </div>
        </div>
        <div class="page-btn">
    ...
    <div class="card shadow-sm border-0">
        <form action="{{ route('admin.vehicle-documents.update', $document->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="card-body">
                @if ($errors->any())
                        ...
                        <div class="mb-4">
                            ...
                            <select name="vehicle_id" class="form-select" required>
                                @foreach($vehicles as $vehicle)
                                    <option value="{{ $vehicle->id }}" {{ old('vehicle_id', $document->vehicle_id) == $vehicle->id ? 'selected' : '' }}>
